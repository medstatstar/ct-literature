...
- **Result cap:** The default `max_results` is **30 works per source**; there is no hard ceiling (raising it is allowed), but time and API usage scale linearly. **Measured on 2026-08-14** (two sources, verification off): `--max 30` → **~5 s**, `--max 100` → **~9 s**, `--max 200` → **~14 s** (≈0.05 s/work). **Suggested ceiling: 200 works per source per run** (≈600 merged across 3 sources; full citation verification on top adds ~1 min per ~60 works with the default 24-worker pool). Beyond 200 the yield per minute drops sharply — for very large harvests, configure an OpenAlex key and use `--verify top 15` instead of pushing `max_results`.
- **Rate limits:**
...
**Q: Can I search in Chinese?** A: Partially — the skill auto-translates Chinese topics to English through a **bundled offline dictionary** (~650 medical terms + drug names, no network call) before querying the APIs, and the report banner shows the original and the translation as `中文 → English`. Terms the dictionary does not cover are passed through as-is (recall may suffer) and a notice lists which ones were not mapped. For best recall, use English terms — especially for rare conditions or novel compounds.
